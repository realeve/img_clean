...
export const getType: (data: any) => TypeList = (data) =>
  R.type(data).toLowerCase() as TypeList;
...
export const handleData: <T extends IAxiosState>({
  data,
}: AxiosResponse<T>) => T = ({ data }) => {
  // 刷新token
...
export const axios: <T = TAxiosData>(
  params: IAxiosConfig,
) => Promise<IAxiosState<T>> = async ({ baseURL = host, ..._option }) => {
